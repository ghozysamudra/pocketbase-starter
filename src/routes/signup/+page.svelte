<script lang="ts">
	...
	import { User, Mail, Lock, UserPlus, AlertCircle } from '@lucide/svelte';
	...
	async function handleSignup(e: SubmitEvent) {
		...
		try {
			// 1. Create the user record in PocketBase users collection
			...
			errorMessage = error?.message || 'Failed to create account. Please check details and try again.';
		} finally {
			isLoading = false;
		}
	}
...
<svelte:head>
	<title>Sign Up - PocketBase App</title>
...
<div class="relative flex min-h-[85vh] items-center justify-center px-4 py-12 sm:px-6 lg:px-8 overflow-hidden bg-background">
	...
	<Card.Root class="w-full max-w-md border-border/40 bg-card/60 backdrop-blur-md shadow-2xl transition-all duration-300 hover:shadow-primary/5">
		<Card.Header class="space-y-1 text-center">
			<Card.Title class="text-3xl font-bold tracking-tight bg-gradient-to-r from-primary to-primary/80 bg-clip-text text-transparent">Create an account</Card.Title>
		...
		<form onsubmit={handleSignup}>
			<Card.Content class="grid gap-4">
				...
				<div class="grid gap-2">
					<Label for="name" class="text-sm font-medium">Full Name (optional)</Label>
					<div class="relative">
			...
			<Card.Footer class="flex flex-col gap-4">
				...
				<div class="text-center text-sm text-muted-foreground">
					...
					<a href="/login" class="font-medium text-primary hover:underline underline-offset-4">
						Sign in
					</a>
				</div>
			</Card.Footer>
